...
class Coordinate extends Controller
{
    ...
    {
        $data = $request->input('nmea_data');
        Log::info('Received NMEA data: ' . $data);

        return view('nmea', ['nmeaData' => $data]);
        // return response()->json(['message' => 'NMEA data received']);
    }
    ...
    public function insertCoorGGA(){
        $message = "";
        $call_sign = request()->call_sign;
        $coor_gga = request()->coor;
        $gps_quality = [
            'Fix not valid','GPS fix',
            'Differential GPS fix (DGNSS), SBAS, OmniSTAR VBS, Beacon, RTX in GVBS mode',
            'Not applicable',
        ...
        try {
            ...
            $findCoor = ModelsCoordinate::where("call_sign",$call_sign)->whereNull('id_coor_gga')->orderBy('series_id',"ASC")->first();
            if($findCoor){
    ...
    public function insertCoorHDT(){
        $message = "";
        $call_sign = request()->call_sign;
        $coor_hdt = request()->coor;
        $hdt_split = explode(',',$coor_hdt);
        $validator = Validator::make(request()->all(),[
            'call_sign' => ['required','max:255'],
            'coor' => ['required','max:255'],
